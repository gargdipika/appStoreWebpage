...
class AppStore extends Component {
  state = {
    searchInput: '',
    appList: appsList.filter(eachApp => eachApp.category === 'SOCIAL'),
    tbId: 'SOCIAL',
  }

  changeItemAccTab = tabId => {
    this.setState({
      appList: appsList.filter(eachApp => eachApp.category === tabId),
      tbId: tabId,
    })
  }
  ...
  render() {
    const {searchInput, appList, tbId} = this.state
    const filterAppList = appList.filter(eachApp =>
        ...
        <ul className="tab-list">
          {tabsList.map(eachTab => (
            <TabItem
              key={eachTab.tabId}
              tabDetail={eachTab}
              isClick={eachTab.tabId === tbId}
              onChangeSearch={this.changeItemAccTab}
            />
          ))}
        </ul>
